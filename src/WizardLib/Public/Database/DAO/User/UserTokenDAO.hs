...
findApiUserTokensWithCloseExpiration = do
  ...
  let sql =
        f'
          ...
          [table, show ApiKeyUserTokenType]
...
findUserTokensByUserUuidAndType userUuid aType = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesByFn table [tenantQueryUuid tenantUuid, ("user_uuid", U.toString userUuid), ("type", aType)]

findUserTokensBySessionState :: AppContextC s sc m => String -> m [UserToken]
findUserTokensBySessionState sessionState = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesByFn table [tenantQueryUuid tenantUuid, ("session_state", sessionState)]

findUserTokensByValue :: AppContextC s sc m => String -> m [UserToken]
findUserTokensByValue value = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesByFn table [tenantQueryUuid tenantUuid, ("value", value)]
...
findUserTokenByUuid uuid = getFromCacheOrDb getFromCache addToCache go (U.toString uuid)
  where
    go uuid = do
      tenantUuid <- asks (.tenantUuid')
      table <- tableName entityName
      createFindEntityByFn table [tenantQueryUuid tenantUuid, ("uuid", uuid)]
...
insertUserToken userToken = do
  table <- tableName entityName
  result <- createInsertFn table userToken
...
deleteUserTokensWithExpiration = do
  table <- tableName entityName
  let sql = f' "DELETE FROM %s WHERE expires_at <= now()" [table]
  logInfoI _CMP_DATABASE (trim sql)
...
deleteUserTokenByUuid uuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  result <- createDeleteEntityByFn table [tenantQueryUuid tenantUuid, ("uuid", U.toString uuid)]
