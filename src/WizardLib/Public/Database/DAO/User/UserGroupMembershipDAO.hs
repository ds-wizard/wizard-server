...
findUserGroupMembershipsByUserGroupUuid userGroupUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesBySortedFn table [tenantQueryUuid tenantUuid, ("user_group_uuid", U.toString userGroupUuid)] [Sort "user_uuid" Ascending]
...
findUserGroupMembershipByUserGroupUuidAndUserUuid' userGroupUuid userUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntityByFn' table [tenantQueryUuid tenantUuid, ("user_group_uuid", U.toString userGroupUuid), ("user_uuid", U.toString userUuid)]

insertUserGroupMembership :: AppContextC s sc m => UserGroupMembership -> m Int64
insertUserGroupMembership userGroupMembership = do
  table <- tableName entityName
  createInsertFn table userGroupMembership
...
deleteUserGroupMembershipsByUserGroupUuid userGroupUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createDeleteEntityByFn table [tenantQueryUuid tenantUuid, ("user_group_uuid", U.toString userGroupUuid)]
  return ()
  ...
  let sql =
        fromString $
          f'
            ...
            [table, generateQuestionMarks userUuids]
