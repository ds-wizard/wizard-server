...
runIn mServerUrl mUser transactionState function = do
  ...
  if tenant.enabled
    then do
      baseContext <- ask
      eResult <-
        liftIO $ appContextFromBaseContext tenant.uuid mUser transactionState baseContext $ \appContext ->
          guardAppContext appContext (runMonads (runAppContextM function) appContext)
      case eResult of
...
runWithSystemUser function = do
  baseContext <- ask
  eResult <-
    liftIO $ appContextFromBaseContext defaultTenantUuid (Just . toDTO $ userSystem) NoTransaction baseContext $ \appContext ->
      guardAppContext appContext (runMonads (runAppContextM function) appContext)
  case eResult of
    Right result -> return result
    Left error -> throwError =<< sendError error
