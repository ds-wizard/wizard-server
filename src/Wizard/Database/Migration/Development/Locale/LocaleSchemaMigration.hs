...
dropTables = do
  ...
  let sql =
        "DROP TABLE IF EXISTS w_locale CASCADE;"
...
dropTriggers = do
  ...
  let sql = "DROP TRIGGER IF EXISTS trg_locale_after_delete_s3 ON w_locale;"
  let action conn = execute_ conn sql
  runDB action
...
createTables = do
  ...
  let sql =
        ...
        \    CONSTRAINT w_locale_pk PRIMARY KEY (uuid),\
        \    CONSTRAINT w_locale_tenant_uuid_fk FOREIGN KEY (tenant_uuid) REFERENCES w_tenant (uuid) ON DELETE CASCADE \
        \);"
  let action conn = execute_ conn sql
  runDB action
...
createTriggers = do
  ...
  let sql =
        ...
        \    ON w_locale \
        \    FOR EACH ROW \
        \EXECUTE FUNCTION w_create_persistent_command_from_entity_uuid('locale', 'deleteFromS3');"
  let action conn = execute_ conn sql
  runDB action
