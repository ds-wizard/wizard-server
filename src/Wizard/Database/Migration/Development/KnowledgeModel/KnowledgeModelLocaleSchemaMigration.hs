...
dropTriggers = do
  ...
  let sql = "DROP TRIGGER IF EXISTS trg_knowledge_model_locale_after_delete_s3 ON w_knowledge_model_locale;"
...
dropTables = do
  ...
  let sql = "DROP TABLE IF EXISTS w_knowledge_model_locale CASCADE;"
  let action conn = execute_ conn sql
  runDB action
...
createTriggers = do
  ...
  let sql =
        ...
        \    AFTER DELETE \
        \    ON w_knowledge_model_locale \
        \    FOR EACH ROW \
        \EXECUTE FUNCTION w_create_persistent_command_from_entity_uuid('knowledge_model_locale', 'deleteFromS3');"
  let action conn = execute_ conn sql
  runDB action
