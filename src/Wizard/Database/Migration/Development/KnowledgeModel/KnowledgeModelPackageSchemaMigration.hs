...
dropTables = do
  ...
  let sql =
        "DROP TABLE IF EXISTS w_knowledge_model_package_event CASCADE; \
        \DROP TABLE IF EXISTS w_knowledge_model_package CASCADE;"
...
dropFunctions = do
  ...
  let sql =
        "DROP FUNCTION IF EXISTS w_get_newest_knowledge_model_package; \
        \DROP FUNCTION IF EXISTS w_get_newest_knowledge_model_package_coordinate;\
        \DROP FUNCTION IF EXISTS w_get_organization_id;\
        \DROP FUNCTION IF EXISTS w_get_km_id;"
  let action conn = execute_ conn sql
  runDB action
...
createKnowledgeModelPackageTable = do
  ...
  let sql =
        ...
        \); \
        \ \
        \CREATE INDEX w_knowledge_model_package_organization_id_km_id_index ON w_knowledge_model_package (organization_id, km_id, tenant_uuid); \
        \ \
        \CREATE INDEX w_knowledge_model_package_previous_package_uuid_index ON w_knowledge_model_package (previous_package_uuid);"
  let action conn = execute_ conn sql
  runDB action
...
createKnowledgeModelPackageEventTable = do
  ...
  let sql =
        "CREATE TABLE IF NOT EXISTS w_knowledge_model_package_event \
        \( \
        \    uuid         uuid        NOT NULL, \
...
createGetNewestPackageFn = do
  let sql =
        "CREATE OR REPLACE FUNCTION w_get_newest_knowledge_model_package(req_organization_id varchar, req_km_id varchar, req_tenant_uuid uuid, req_phase varchar[]) \
        \    RETURNS uuid \
        \    LANGUAGE plpgsql \
        \AS \
        \$$ \
...
createGetNewestPackageCoordinateFn = do
  let sql =
        "CREATE or REPLACE FUNCTION w_get_newest_knowledge_model_package_coordinate(req_coordinate varchar, req_tenant_uuid uuid, req_phase varchar[]) \
        \    RETURNS varchar \
        \    LANGUAGE plpgsql \
        \AS \
        \$$ \
        ...
        \    END IF; \
        \ \
        \    target_uuid := w_get_newest_knowledge_model_package( \
        \            w_get_organization_id(req_coordinate), \
        \            w_get_km_id(req_coordinate), \
        \            req_tenant_uuid, \
        \            req_phase \
        \                   ); \
        \ \
...
createGetOrganizationIdFn = do
  let sql =
        "CREATE or REPLACE FUNCTION w_get_organization_id(req_p_id varchar) \
        \    RETURNS varchar \
        \    LANGUAGE plpgsql \
        \AS \
        \$$ \
...
createGetKmIdFn = do
  let sql =
        "CREATE or REPLACE FUNCTION w_get_km_id(req_p_id varchar) \
        \    RETURNS varchar \
        \    LANGUAGE plpgsql \
        \AS \
        \$$ \
