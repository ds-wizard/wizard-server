...
dropTables = do
  ...
  let sql =
        "DROP TABLE IF EXISTS w_document_template_draft_data CASCADE;\
        \DROP TABLE IF EXISTS w_document_template_asset CASCADE;\
        \DROP TABLE IF EXISTS w_document_template_file CASCADE;\
        \DROP TABLE IF EXISTS w_document_template_format_step CASCADE;\
        \DROP TABLE IF EXISTS w_document_template_format CASCADE;\
        \DROP TABLE IF EXISTS w_document_template CASCADE;"
...
dropTriggers = do
  ...
  let sql = "DROP TRIGGER IF EXISTS trigger_on_after_document_template_asset_delete ON w_project_file;"
  let action conn = execute_ conn sql
  runDB action
...
createDraftDataTable = do
  ...
  let sql =
        ...
        \    CONSTRAINT w_document_template_draft_data_pk PRIMARY KEY (document_template_uuid), \
...
createPersistentCommandFromDocumentTemplateAssetDeleteFunction = do
  let sql =
        "CREATE OR REPLACE FUNCTION w_create_persistent_command_from_document_template_asset_delete() \
        \    RETURNS TRIGGER AS \
        \$$ \
        \BEGIN \
...
createTriggers = do
  ...
  let sql =
        ...
        \    AFTER DELETE \
        \    ON w_document_template_asset \
        \    FOR EACH ROW \
        \EXECUTE FUNCTION w_create_persistent_command_from_document_template_asset_delete();"
  let action conn = execute_ conn sql
  runDB action
