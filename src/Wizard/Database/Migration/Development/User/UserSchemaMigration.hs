...
dropTables = do
  ...
  let sql =
        "DROP TABLE IF EXISTS w_user_plugin_settings CASCADE;\
        \DROP TABLE IF EXISTS w_user_tour CASCADE;\
        \DROP TABLE IF EXISTS w_user_group_membership CASCADE;\
        \DROP TABLE IF EXISTS w_user_group CASCADE;\
        \DROP TABLE IF EXISTS w_user_token CASCADE;\
        \DROP TABLE IF EXISTS w_user_entity_submission_prop CASCADE; \
        \DROP TABLE IF EXISTS w_user_entity CASCADE;"
...
createUserTable = do
  ...
  let sql =
        ...
        \); \
        \ \
        \CREATE UNIQUE INDEX w_user_email_uindex ON w_user_entity (email, tenant_uuid);"
  let action conn = execute_ conn sql
  runDB action
...
createUserLocaleForeignKeyConstraint = do
  logInfo _CMP_MIGRATION "(Table/User) create tables"
  let sql = "ALTER TABLE w_user_entity ADD CONSTRAINT w_user_entity_locale_fk FOREIGN KEY (locale) REFERENCES w_locale (uuid) ON DELETE SET NULL;"
  let action conn = execute_ conn sql
  runDB action
