...
updateFeedbackByUuid feedback = do
  ...
  let sql =
        fromString
          "UPDATE w_feedback SET uuid = ?, issue_id = ?, question_uuid = ?, knowledge_model_package_uuid = ?, title = ?, content = ?, created_at = ?, updated_at = ?, tenant_uuid = ? WHERE tenant_uuid = ? AND uuid = ?"
