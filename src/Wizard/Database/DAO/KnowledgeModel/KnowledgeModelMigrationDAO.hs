...
entityName = "w_knowledge_model_migration"
...
updateKnowledgeModelMigration migration = do
  let sql =
        fromString
          "UPDATE w_knowledge_model_migration SET editor_uuid = ?, metamodel_version = ?, state = ?, editor_previous_package_uuid = ?, target_package_uuid = ?, editor_previous_package_events = ?, target_package_events = ?, result_events = ?, current_knowledge_model = ?, tenant_uuid = ?, created_at = ? WHERE tenant_uuid = ? AND editor_uuid = ?"
