...
entityName = "w_knowledge_model_editor"
...
findKnowledgeModelEditorsPage mQuery pageable sort =
  ...
  do
    ...
    let sql =
          fromString $
            f'
              "SELECT w_knowledge_model_editor.uuid, \
              \       w_knowledge_model_editor.name, \
              \       w_knowledge_model_editor.km_id, \
              \       w_knowledge_model_editor.version, \
              \       w_get_knowledge_model_editor_state(w_knowledge_model_editor, w_knowledge_model_migration, w_get_knowledge_model_editor_fork_of_package_id(w_config_organization, previous_pkg, w_knowledge_model_editor), '%s') as state, \
              \       w_knowledge_model_editor.previous_package_uuid, \
              ...
              \       w_knowledge_model_editor.created_by, \
              \       w_knowledge_model_editor.created_at, \
              \       GREATEST(w_knowledge_model_editor.updated_at, (SELECT MAX(created_at) FROM w_knowledge_model_editor_event WHERE w_knowledge_model_editor_event.editor_uuid = w_knowledge_model_editor.uuid AND w_knowledge_model_editor_event.tenant_uuid = w_knowledge_model_editor.tenant_uuid)) AS updated_at  \
              \FROM w_knowledge_model_editor \
              \JOIN w_config_organization ON w_knowledge_model_editor.tenant_uuid = w_config_organization.tenant_uuid \
              \LEFT JOIN w_knowledge_model_migration ON w_knowledge_model_editor.uuid = w_knowledge_model_migration.editor_uuid \
              \LEFT JOIN w_knowledge_model_package previous_pkg ON w_knowledge_model_editor.previous_package_uuid = previous_pkg.uuid and w_knowledge_model_editor.tenant_uuid = previous_pkg.tenant_uuid \
              \LEFT JOIN w_knowledge_model_package fork_of_package ON w_get_knowledge_model_editor_fork_of_package_id(w_config_organization, previous_pkg, w_knowledge_model_editor) = concat(fork_of_package.organization_id, ':', fork_of_package.km_id, ':', fork_of_package.version) AND w_knowledge_model_editor.tenant_uuid = fork_of_package.tenant_uuid \
              \WHERE (w_knowledge_model_editor.name ~* ? OR w_knowledge_model_editor.km_id ~* ?) AND w_knowledge_model_editor.tenant_uuid = ? \
              \%s OFFSET %s LIMIT %s"
...
findEditorsByUnsupportedMetamodelVersion metamodelVersion = do
  ...
  let sql = fromString "SELECT * FROM w_knowledge_model_editor WHERE metamodel_version != ? AND tenant_uuid = ?"
...
findEditorsForSquashing = do
  let sql = "SELECT uuid FROM w_knowledge_model_editor WHERE squashed = false"
...
updateKnowledgeModelEditorByUuid kmEditor = do
  ...
  let sql =
        fromString
          "UPDATE w_knowledge_model_editor SET uuid = ?, name = ?, km_id = ?, previous_package_uuid = ?, created_by = ?, created_at = ?, updated_at = ?, tenant_uuid = ?, version = ?, description = ?, readme = ?, license = ?, metamodel_version = ?, squashed = ?, language = ? WHERE tenant_uuid = ? AND uuid = ?;"
...
updateKnowledgeModelEditorMetamodelVersion uuid metamodelVersion = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_knowledge_model_editor SET metamodel_version = ? WHERE tenant_uuid = ? AND uuid = ?"
