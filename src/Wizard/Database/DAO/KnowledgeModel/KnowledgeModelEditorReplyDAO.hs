...
entityName = "w_knowledge_model_editor_reply"
    ...
    (editorReply : _) -> do
      ...
      let sql =
            fromString $
              f'
                "BEGIN TRANSACTION; \
                \DELETE FROM w_knowledge_model_editor_reply WHERE tenant_uuid = ? AND editor_uuid = ?; \
                \INSERT INTO w_knowledge_model_editor_reply (path, value_type, value, value_raw, editor_uuid, created_by, tenant_uuid, created_at) \
                \VALUES %s; \
                \COMMIT;"
                [insertSql]
      let params =
