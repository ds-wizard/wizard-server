...
entityName = "w_knowledge_model_package"
...
findPackagesPage mOrganizationId mKmId mQuery mOutdated pageable sort =
  createFindEntitiesGroupByCoordinatePageableQuerySortFn
    ...
    pageable
    sort
    "uuid, w_knowledge_model_package.name, w_knowledge_model_package.organization_id, w_knowledge_model_package.km_id, version, phase, description, non_editable, public, w_registry_knowledge_model_package.remote_version, w_registry_organization.name as org_name, w_registry_organization.logo as org_logo, w_knowledge_model_package.language, w_knowledge_model_package.created_at"
    "km_id"
    mQuery
    Nothing
    mOrganizationId
    ...
    mOutdated
    ( case mOutdated of
        Just _ -> " AND w_is_outdated(w_registry_knowledge_model_package.remote_version, version) = ?"
        Nothing -> ""
    )
    ...
    let sql =
          fromString $
            f'
              ...
              \       version, \
              \       description \
              \FROM w_knowledge_model_package outer_package \
...
findDependentPackageResources pkgUuids = do
  ...
  let sql =
        f''
          ...
          \        p.tenant_uuid \
          \    FROM w_knowledge_model_package p \
          ...
          \    )), '[]'::jsonb) \
          \    FROM w_knowledge_model_package child \
          ...
          \        'name', e.name \
          \    )), '[]'::jsonb) \
          \    FROM w_knowledge_model_editor e \
...
updatePackageMetamodelVersion pkgUuid metamodelVersion = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_knowledge_model_package SET metamodel_version = ? WHERE tenant_uuid = ? AND uuid = ?"
...
updatePackagePhaseAndPublicByUuid pkgUuid phase public = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_knowledge_model_package SET phase = ?, public = ? WHERE tenant_uuid = ? AND uuid = ?"
