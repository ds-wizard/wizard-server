...
entityName = "w_user_plugin_settings"
...
updateUserPluginSettings pluginSettings = do
  let sql =
        fromString
          "UPDATE w_user_plugin_settings SET user_uuid = ?, plugin_uuid = ?,  values = ?,  tenant_uuid = ?,  created_at = ?, updated_at = ? WHERE user_uuid = ? AND plugin_uuid = ?"
