...
findUserSuggestionsPage mQuery mSelectUuids mExcludeUuids pageable sort = do
  ...
  createFindEntitiesPageableQuerySortFn
    ...
    pageable
    sort
    "uuid, first_name, last_name, w_gravatar_hash(email), image_url, affiliation"
    condition
...
findUsersByUserGroupUuid userGroupUuid = do
  ...
  let sql =
        fromString
          ...
          \FROM w_user_group_membership ugm \
          \JOIN w_user_entity u ON u.uuid = ugm.user_uuid AND u.tenant_uuid = ugm.tenant_uuid \
...
countUsersByRole roleUuid = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "SELECT COUNT(*) FROM w_user_entity WHERE role_uuid = ? AND tenant_uuid = ?"
...
insertUser user = do
  ...
  let sql =
        fromString
          "INSERT INTO w_user_entity VALUES (?, ?, ?, ?, ?, ?, ?, ?::varchar[], ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
updateUserByUuid user = do
  let sql =
        fromString
          "UPDATE w_user_entity SET uuid = ?, first_name = ?, last_name = ?, email = ?, password_hash = ?, affiliation = ?, role_uuid = ?, role_permissions = ?, active = ?, image_url = ?, last_visited_at = ?, created_at = ?, updated_at = ?, tenant_uuid = ?, machine = ?, locale = ?, last_seen_news_id = ?, email_verified_at = ?, email_pending = ?, role_name = ? WHERE tenant_uuid = ? AND uuid = ?"
...
updateUserPasswordByUuid userUuid uPassword uUpdatedAt = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_user_entity SET password_hash = ?, updated_at = ? WHERE tenant_uuid = ? AND uuid = ?"
...
updateUserLastVisitedAtByUuid userUuid lastVisitedAt = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_user_entity SET last_visited_at = ? WHERE tenant_uuid = ? AND uuid = ?"
...
updateUserLocaleByUuid userUuid mLocale uUpdatedAt = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_user_entity SET locale = ?, updated_at = ? WHERE tenant_uuid = ? AND uuid = ?"
...
updateUserLastSeenNewsIdUuid userUuid lastSeenNewsId = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_user_entity SET last_seen_news_id = ? WHERE tenant_uuid = ? AND uuid = ?"
...
updateUsersRoleByRole roleUuid permissions roleName = do
  ...
  let sql =
        fromString
          "UPDATE w_user_entity SET role_permissions = ?, role_name = ? WHERE role_uuid = ? AND tenant_uuid = ?"
