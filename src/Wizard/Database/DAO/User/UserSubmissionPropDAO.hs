...
entityName = "w_user_entity_submission_prop"
...
findUserSubmissionPropsList userUuid = do
  ...
  let sql =
        fromString
          ...
          \      FROM w_config_submission_service AS service \
          \      WHERE service.tenant_uuid = ?) AS nested \
          \LEFT JOIN w_user_entity_submission_prop AS submission_prop \
...
insertOrUpdateUserSubmissionProp submissionProps = do
  let sql =
        fromString
          "INSERT INTO w_user_entity_submission_prop \
          \VALUES (?, ?, ?, ?, ?, ?) \
...
deleteUserSubmissionPropsExcept userUuid serviceIds = do
  ...
  let sql =
        fromString $
          f'
            "DELETE FROM w_user_entity_submission_prop \
