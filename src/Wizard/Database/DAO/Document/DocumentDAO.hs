  ...
  do
    ...
    let (projectSelect, projectSelectParams, projectJoin, projectCondition, projectParam) =
          case (mProjectUuid, mProjectName) of
            ...
            (Just projectUuid, Nothing) -> ("w_project.name, ", [], "LEFT JOIN w_project ON w_project.uuid = doc.project_uuid", "AND doc.project_uuid = ?", [U.toString projectUuid])
            _ -> ("w_project.name, ", [], "LEFT JOIN w_project ON w_project.uuid = doc.project_uuid", "", [])
    ...
    -- 2. Get total count
    count <- createCountByFn "w_document doc" condition baseParams
    -- 3. Get entities
    let sql =
          fromString $
            f''
              ...
              \FROM w_document doc \
              \${projectJoin} \
              \LEFT JOIN w_document_template doc_tml ON doc_tml.uuid = doc.document_template_uuid AND doc_tml.tenant_uuid = doc.tenant_uuid \
              \LEFT JOIN w_document_template_format dt_format ON dt_format.tenant_uuid = doc.tenant_uuid AND dt_format.document_template_uuid = doc.document_template_uuid AND dt_format.uuid = doc.format_uuid \
              \LEFT JOIN w_project_version ON w_project_version.event_uuid = doc.project_event_uuid AND w_project_version.tenant_uuid = doc.tenant_uuid \
              \${condition} \
              \${sort} \
              \OFFSET ${offset} \
              \LIMIT ${limit}"
...
deleteTemporalDocumentsByAssetUuid = deleteTemporalDocumentsByTableAndUuid "w_document_template_asset"

deleteTemporalDocumentsByFileUuid :: U.UUID -> AppContextM Int64
deleteTemporalDocumentsByFileUuid = deleteTemporalDocumentsByTableAndUuid "w_document_template_file"
  ...
  let sql =
        fromString $
          f'
            ...
            \    SELECT d.uuid \
            \    FROM %s join_table \
            \             JOIN w_document d ON join_table.document_template_uuid = d.document_template_uuid \
