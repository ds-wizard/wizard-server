...
updatePluginEnabled uuid enabled = do
  ...
  let sql = fromString "UPDATE w_plugin SET enabled = ?, updated_at = now() WHERE tenant_uuid = ? AND uuid = ?"
...
updatePluginUrlForAllTenants uuid url = do
  let sql = fromString "UPDATE w_plugin SET url = ?, updated_at = now() WHERE uuid = ?"
...
updatePluginUrlForTenant tenantUuid uuid url = do
  let sql = fromString "UPDATE w_plugin SET url = ?, updated_at = now() WHERE tenant_uuid = ? AND uuid = ?"
