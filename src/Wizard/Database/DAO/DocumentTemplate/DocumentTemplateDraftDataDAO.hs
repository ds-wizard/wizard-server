...
entityName = "w_document_template_draft_data"
...
updateDraftDataById draftData = do
  ...
  let sql =
        fromString
          "UPDATE w_document_template_draft_data SET document_template_uuid = ?, project_uuid = ?, format_uuid = ?, tenant_uuid = ?, created_at = ?, updated_at = ?, knowledge_model_editor_uuid = ? WHERE tenant_uuid = ? AND document_template_uuid = ?"
