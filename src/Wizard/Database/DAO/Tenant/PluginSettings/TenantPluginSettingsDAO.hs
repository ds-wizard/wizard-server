...
entityName = "w_tenant_plugin_settings"
...
updateTenantPluginSettings pluginSettings = do
  let sql =
        fromString
          "UPDATE w_tenant_plugin_settings SET tenant_uuid = ?,  plugin_uuid = ?,  values = ?,  created_at = ?, updated_at = ? WHERE tenant_uuid = ? AND plugin_uuid = ?"
