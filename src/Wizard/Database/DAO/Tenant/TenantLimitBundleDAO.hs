...
entityName = "w_tenant_limit_bundle"
...
updateLimitBundleByUuid limitBundle = do
  ...
  let sql =
        fromString
          "UPDATE w_tenant_limit_bundle SET uuid = ?, users = ?, active_users = ?, knowledge_models = ?, knowledge_model_editors = ?, document_templates = ?, projects = ?, documents =?, storage = ?, created_at = ?, updated_at = ?, document_template_drafts = ?, locales = ? WHERE uuid = ?"
