...
findTenantConfigSubmissionByUuid tenantUuid = do
  tcSubmission <- createFindEntityByFn "w_config_submission" [("tenant_uuid", U.toString tenantUuid)]
...
insertTenantConfigSubmission submission = do
  createInsertFn "w_config_submission" submission
...
updateTenantConfigSubmission submission = do
  let sql = "UPDATE w_config_submission SET tenant_uuid = ?, enabled = ?, created_at = ?, updated_at = ? WHERE tenant_uuid = ?;"
...
deleteTenantConfigSubmissions = createDeleteEntitiesFn "w_config_submission"
...
findTenantConfigSubmissionServices = do
  tenantUuid <- asks currentTenantUuid
  tcSubmissionServices <- createFindEntitiesByFn "w_config_submission_service" [("tenant_uuid", U.toString tenantUuid)]
  traverse
    ( \service -> do
...
findTenantConfigSubmissionServicesByDocumentTemplateUuidAndFormatUuid documentTemplateUuid formatUuid = do
  ...
  let sql =
        fromString
          ...
          \                service.name, \
          \                service.description \
          \FROM w_config_submission_service service \
          \     JOIN w_config_submission_service_supported_format supported_format \
...
findTenantConfigSubmissionServiceByServiceId serviceId = do
  tenantUuid <- asks currentTenantUuid
  service <- createFindEntityByFn "w_config_submission_service" [("tenant_uuid", U.toString tenantUuid), ("id", serviceId)]
  supportedFormats <- findTenantConfigSubmissionServiceSupportedFormats service.sId
  requestHeaders <- findTenantConfigSubmissionServiceRequestHeaders service.sId
  return $
    service
...
insertOrUpdateConfigSubmissionService service = do
  let sql =
        fromString
          "INSERT INTO w_config_submission_service \
          \VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) \
...
deleteTenantConfigSubmissionsExcept serviceIds = do
  ...
  let sql =
        fromString $
          f'
            "DELETE FROM w_config_submission_service \
            \WHERE tenant_uuid = ? %s"
            [serviceIdCondition]
...
findTenantConfigSubmissionServiceSupportedFormats serviceId = do
  tenantUuid <- asks currentTenantUuid
  createFindEntitiesByFn "w_config_submission_service_supported_format" [("tenant_uuid", U.toString tenantUuid), ("service_id", serviceId)]
...
insertOrUpdateConfigSubmissionServiceSupportedFormat supportedFormat = do
  let sql =
        fromString
          "INSERT INTO w_config_submission_service_supported_format \
          \VALUES (?, ?, ?, ?) \
...
deleteTenantConfigSubmissionsSupportedFormatExcept serviceId supportedFormats = do
  ...
  let sql = fromString $ f' "DELETE FROM w_config_submission_service_supported_format WHERE tenant_uuid = ? AND service_id = ? %s" [condition]
...
findTenantConfigSubmissionServiceRequestHeaders serviceId = do
  ...
  let sql =
        fromString
          "SELECT name, value \
          \FROM w_config_submission_service_request_header \
...
insertOrUpdateConfigSubmissionServiceRequestHeader tenantUuid serviceId (name, value) = do
  let sql =
        fromString
          "INSERT INTO w_config_submission_service_request_header \
          \VALUES (?, ?, ?, ?) \
...
deleteTenantConfigSubmissionsRequestHeaderExcept serviceId names = do
  ...
  let sql = fromString $ f' "DELETE FROM w_config_submission_service_request_header WHERE tenant_uuid = ? AND service_id = ? %s" [condition]
