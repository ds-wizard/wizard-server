...
findTenantConfigAuthenticationByUuid tenantUuid =
  createFindEntityByFn "w_config_authentication" [("tenant_uuid", U.toString tenantUuid)]

insertTenantConfigAuthentication :: TenantConfigAuthentication -> AppContextM Int64
insertTenantConfigAuthentication = createInsertFn "w_config_authentication"

updateTenantConfigAuthentication :: TenantConfigAuthentication -> AppContextM Int64
updateTenantConfigAuthentication config = do
  let sql =
        fromString
          "UPDATE w_config_authentication SET tenant_uuid = ?, default_role_uuid = ?, internal_registration_enabled = ?, internal_two_factor_auth_enabled = ?, internal_two_factor_auth_code_length = ?, internal_two_factor_auth_code_expiration = ?, created_at = ?, updated_at = ?, internal_non_admin_login_enabled = ?, internal_session_expiration = ?, internal_user_email_link_expiration = ? WHERE tenant_uuid = ?"
...
deleteTenantConfigAuthentications = createDeleteEntitiesFn "w_config_authentication"
