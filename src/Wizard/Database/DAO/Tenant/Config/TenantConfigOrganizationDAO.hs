...
entityName = "w_config_organization"
...
updateTenantConfigOrganization config = do
  let sql =
        fromString
          "UPDATE w_config_organization SET tenant_uuid = ?, name = ?, description = ?, organization_id = ?, affiliations = ?, created_at = ?, updated_at = ? WHERE tenant_uuid = ?"
