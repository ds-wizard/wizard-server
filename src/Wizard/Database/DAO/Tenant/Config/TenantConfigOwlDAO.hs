...
updateTenantConfigOwl config = do
  let sql = fromString "UPDATE w_config_owl SET tenant_uuid = ?, enabled = ?, name = ?, organization_id = ?, km_id = ?, version = ?, previous_package_uuid = ?, root_element = ?, created_at = ?, updated_at = ? WHERE tenant_uuid = ?"
