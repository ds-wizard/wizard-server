...
entityName = "w_config_privacy_and_support"
...
updateTenantConfigPrivacyAndSupport config = do
  let sql = fromString "UPDATE w_config_privacy_and_support SET tenant_uuid = ?, privacy_url = ?, terms_of_service_url = ?, support_email = ?, support_site_name = ?, support_site_url = ?, support_site_icon = ?, created_at = ?, updated_at = ? WHERE tenant_uuid = ?"
