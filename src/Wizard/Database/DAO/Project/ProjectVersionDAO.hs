  ...
  let sql =
        fromString $
          f''
            ...
            \       u.image_url, \
            \       u.affiliation \
            \FROM w_project_version v \
            \JOIN w_user_entity u ON u.uuid = v.created_by AND u.tenant_uuid = v.tenant_uuid ${createdAtCondition} \
...
updateProjectVersionByUuid version = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_project_version SET uuid = ?, name = ?, description = ?, event_uuid = ?, project_uuid = ?, tenant_uuid = ?, created_by = ?, created_at = ?, updated_at = ? WHERE uuid = ? AND tenant_uuid = ?"
