  ...
  let sql =
        fromString $
          f'
            ...
            \INSERT INTO %s VALUES (%s); \
            \COMMIT;"
            ["w_project_comment_thread", generateQuestionMarks' thread, entityName, generateQuestionMarks' comment]
...
updateProjectCommentById entity = do
  ...
  let sql =
        fromString
          "UPDATE w_project_comment SET uuid = ?, text = ?, created_by = ?, created_at = ?, updated_at = ?, tenant_uuid = ? WHERE uuid = ? AND tenant_uuid = ?"
...
updateProjectCommentTextById uuid text = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_project_comment SET text = ?, updated_at = now() WHERE uuid = ? AND tenant_uuid = ?"
