...
findProjectFilesPage mQuery mProjectUuid pageable sort = do
  ...
  do
    ...
    -- 2. Get total count
    count <- createCountByFn "w_project_file file" condition conditionParams
    -- 3. Get entities
    let sql =
          fromString $
            f''
              ...
              \FROM w_project_file file \
              \LEFT JOIN w_user_entity created_by ON created_by.uuid = file.created_by AND created_by.tenant_uuid = file.tenant_uuid \
              \LEFT JOIN w_project ON w_project.uuid = file.project_uuid AND w_project.tenant_uuid = file.tenant_uuid \
              \${condition} \
              \${sort} \
              \OFFSET ${offset} \
              \LIMIT ${limit}"
