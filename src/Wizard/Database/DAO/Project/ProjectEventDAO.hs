...
findProjectEventsPage projectUuid pageable sort = do
  ...
  do
    ...
    let sql =
          fromString $
            f''
              "SELECT w_project_event.uuid, \
              \       w_project_event.event_type, \
              \       w_project_event.path, \
              \       w_project_event.created_at, \
              \       w_project_event.value_type, \
              \       w_project_event.value, \
              \       w_project_event.value_raw, \
              \       w_project_event.created_by   AS created_by_uuid, \
              \       w_user_entity.first_name           AS created_by_first_name, \
              \       w_user_entity.last_name            AS created_by_last_name, \
              \       w_gravatar_hash(w_user_entity.email) AS created_by_gravatar_hash, \
              \       w_user_entity.image_url            AS created_by_image_url, \
              \       w_user_entity.affiliation          AS created_by_affiliation \
              \FROM w_project_event \
              \     LEFT JOIN w_user_entity ON w_user_entity.uuid = w_project_event.created_by \
              \WHERE w_project_event.tenant_uuid = ? \
              \  AND w_project_event.project_uuid = ? \
              \${sort} \
              \OFFSET ${offset} \
              \LIMIT ${limit}"
              [ ("sort", mapSort sort)
...
insertProjectEventWithTimestampUpdate projectUuid event = do
  ...
  let sql =
        fromString $
          f'
            "UPDATE w_project SET squashed = false, updated_at = now() WHERE tenant_uuid = ? AND uuid = ?; \
            \INSERT INTO w_project_event VALUES (%s)"
...
updateProjectEventByUuid event = do
  tenantUuid <- asks currentTenantUuid
  let sql = fromString "UPDATE w_project_event SET uuid = ?, event_type = ?, path = ?, created_at = ?, created_by = ?, project_uuid = ?, tenant_uuid = ?, value_type = ?, value = ?, value_raw = ? WHERE uuid = ? AND tenant_uuid = ?"
