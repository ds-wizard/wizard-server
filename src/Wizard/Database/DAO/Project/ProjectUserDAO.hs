    ...
    let countSql =
          fromString $
            f''
              ...
              \      JOIN w_user_entity u ON w_project_perm_user.user_uuid = u.uuid AND w_project_perm_user.tenant_uuid = u.tenant_uuid \
              \      WHERE w_project_perm_user.project_uuid = ? \
              \        AND w_project_perm_user.tenant_uuid = ? \
              \        AND w_project_perm_user.perms @> ARRAY ['${perm}'] \
              \        AND u.active = true \
              \        AND u.machine = false \
              \      UNION ALL \
              \      SELECT u.uuid, \
              ...
              \      LEFT JOIN w_user_group_membership ug_membership ON ug_membership.user_group_uuid = w_project_perm_group.user_group_uuid AND ug_membership.tenant_uuid = w_project_perm_group.tenant_uuid \
              \      LEFT JOIN w_user_entity u ON u.uuid = ug_membership.user_uuid AND u.tenant_uuid = w_project_perm_group.tenant_uuid \
              \      WHERE w_project_perm_group.project_uuid = ? \
              \        AND w_project_perm_group.tenant_uuid = ? \
              \        AND w_project_perm_group.perms @> ARRAY ['${perm}'] \
              \        AND u.active = true \
