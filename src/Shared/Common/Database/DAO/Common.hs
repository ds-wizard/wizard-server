...
tablePrefix :: AppContextC s sc m => m String
tablePrefix = do
  context <- ask
  return context.tablePrefix'

tableName :: AppContextC s sc m => String -> m String
tableName name = do
  prefix <- tablePrefix
  return $ prefix ++ name

entityLabel :: AppContextC s sc m => String -> m String
entityLabel name = do
  prefix <- tablePrefix
  return $ fromMaybe name (L.stripPrefix prefix name)
...
runOneEntityDB entityName action queryParams = do
  ...
  case entities of
    [] -> do
      ...
      throwError $ NotExistsError (_ERROR_DATABASE__ENTITY_NOT_FOUND label queryParams)
