...
-- The prefix keeps a second database in the same config (Analytics' reporting database)
-- off the DATABASE_* variables that configure the operational one.
applyDatabaseEnv prefix serverConfig =
  applyEnvVariables
    serverConfig
    [ \c -> applyStringEnvVariable (prefix ++ "DATABASE_CONNECTION_STRING") c.connectionString (\x -> c {connectionString = x})
    , \c -> applyEnvVariable (prefix ++ "DATABASE_STRIPE_SIZE") c.stripeSize (\x -> c {stripeSize = x})
    , \c -> applyEnvVariable (prefix ++ "DATABASE_CONNECTION_TIMEOUT") c.connectionTimeout (\x -> c {connectionTimeout = x})
    , \c -> applyEnvVariable (prefix ++ "DATABASE_MAX_CONNECTIONS") c.maxConnections (\x -> c {maxConnections = x})
    , \c -> applyEnvVariable (prefix ++ "DATABASE_USE_DEV_MIGRATION") c.useDevMigration (\x -> c {useDevMigration = x})
    ]

instance FromEnv ServerConfigDatabase where
  applyEnv = applyDatabaseEnv ""
...
instance FromEnv ServerConfigSentry where
  applyEnv serverConfig =
    applyEnvVariables
      ...
      , \c -> applyStringEnvVariable "SENTRY_ENVIRONMENT" c.environment (\x -> c {environment = x} :: ServerConfigSentry)
      ]
