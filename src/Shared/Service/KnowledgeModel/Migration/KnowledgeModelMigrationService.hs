module Shared.Service.KnowledgeModel.Migration.KnowledgeModelMigrationService where
...
import Shared.Service.KnowledgeModel.Migration.KnowledgeModelMigrationMapper
...
getCurrentMigration :: WizardRequestContextC s m => U.UUID -> m KnowledgeModelMigration
getCurrentMigration editorUuid = do
  ...
  let stateWithEvent =
        case ms.state of
          ...
          state -> state
  return $ ms {currentKnowledgeModel = Just knowledgeModel, state = stateWithEvent}
...
deleteCurrentMigration :: WizardRequestContextC s m => U.UUID -> m ()
...
solveConflictAndMigrate :: WizardRequestContextC s m => U.UUID -> KnowledgeModelMigrationResolutionDTO -> m ()
...
solveAllConflicts :: WizardRequestContextC s m => U.UUID -> m ()
...
migrateState :: WizardRequestContextC s m => KnowledgeModelMigration -> m KnowledgeModelMigration
migrateState ms =
  runInTransaction $ do
