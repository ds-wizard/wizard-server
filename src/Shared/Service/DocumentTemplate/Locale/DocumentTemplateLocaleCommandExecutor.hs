module Shared.Service.DocumentTemplate.Locale.DocumentTemplateLocaleCommandExecutor where
...
cComponent = "document_template_locale"
...
cDeleteFromS3 persistentCommand = do
  ...
  case eCommand of
    Right command -> do
      removeDocumentTemplateLocale command.uuid
