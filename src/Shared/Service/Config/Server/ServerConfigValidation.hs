module Shared.Service.Config.Server.ServerConfigValidation where
...
validateDatabaseMaxConnections
  :: HasField "database" serverConfig ServerConfigDatabase
  => serverConfig
  -> Either AppError serverConfig
validateDatabaseMaxConnections config
  | config.database.stripeSize * config.database.maxConnections < 8 =
      Left . GeneralServerError $ _ERROR_SERVICE_CONFIG__VALIDATION_DATABASE_MAX_CONNECTIONS
  | otherwise = Right config
