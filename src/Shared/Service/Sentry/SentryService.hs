module Shared.Service.Sentry.SentryService where
...
guardRequestContext
  ...
  -> IO (Either AppError a)
guardRequestContext context action = do
  result <- try action
  case result of
    ...
    Left exception -> rethrowWithRequestContext context exception

rethrowWithRequestContext
  :: ( HasField "serverConfig'" context sc
     , HasField "sentry'" sc ServerConfigSentry
     , HasField "buildInfoConfig'" context BuildInfoConfig
     , HasField "breadcrumbs'" context (IORef [SentryBreadcrumb])
