module Shared.Service.Dev.DevOperationService where

import Control.Monad.Except (throwError)

import Shared.Api.Resource.Dev.DevExecutionDTO
import Shared.Api.Resource.Dev.DevExecutionResultDTO
import Shared.Api.Resource.Dev.DevSectionDTO
import Shared.Localization.Messages.Internal
import Shared.Model.Context.RequestContext
import Shared.Model.Dev.Dev
import Shared.Model.Error.Error
import Shared.Model.User.RolePermission
import Shared.Service.Acl.AclService
import Shared.Service.Dev.DevOperationMapper

getDevOperations :: RequestContextC s sc m => [DevSection m] -> m [DevSectionDTO]
getDevOperations sections = do
  checkPermission _DEV_USE_ROLE_PERMISSION
  return . fmap toDevSectionDTO $ sections

executeOperation :: RequestContextC s sc m => [DevSection m] -> DevExecutionDTO -> m AdminExecutionResultDTO
executeOperation sections reqDto = do
  checkPermission _DEV_USE_ROLE_PERMISSION
  case filter (\s -> s.name == reqDto.sectionName) sections of
    [section] ->
      case filter (\o -> o.name == reqDto.operationName) section.operations of
        [operation] -> do
          result <- operation.function reqDto
          return . AdminExecutionResultDTO $ result
        _ -> throwError . GeneralServerError $ _ERROR_SERVICE_DEV__INVALID_DEV_OPERATION_DEFINITIONS_OR_REQUEST
    _ -> throwError . GeneralServerError $ _ERROR_SERVICE_DEV__INVALID_DEV_OPERATION_DEFINITIONS_OR_REQUEST
