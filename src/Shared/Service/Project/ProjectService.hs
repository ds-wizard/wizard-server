...
import Shared.Model.Coordinate.Coordinate (Coordinate)
...
import Shared.Service.Project.ProjectAudit
...
createProject :: WizardRequestContextC s m => ProjectCreateDTO -> m ProjectDTO
...
createProjectFromTemplate :: WizardRequestContextC s m => ProjectCreateFromTemplateDTO -> m ProjectDTO
...
cloneProject :: WizardRequestContextC s m => U.UUID -> m ProjectDTO
...
createProjectsFromCommands :: WizardRequestContextC s m => [CreateProjectCommand] -> m ()
...
compileProjectDetailQuestionnaire project editor commenter = do
  projectEvents <- findProjectEventListsByProjectUuid project.uuid
  unresolvedCommentCounts <-
    if commenter
      then findProjectCommentThreadsSimple project.uuid False editor
      else return M.empty
  resolvedCommentCounts <-
    if commenter
      then findProjectCommentThreadsSimple project.uuid True editor
      else return M.empty
...
getProjectDetailSettingsById projectUuid = do
  ...
  return $
    project
      { knowledgeModelTags = M.elems knowledgeModel.entities.tags
      , availableLocales = fmap KnowledgeModelLocaleMapper.toList availableLocales
      }

getProjectEventsPage :: WizardRequestContextC s m => U.UUID -> Pageable -> [Sort] -> m (Page ProjectEventList)
getProjectEventsPage projectUuid pageable sort = do
  project <- findProjectByUuid projectUuid
  -- Without an explicit ORDER BY the row order is undefined and differs between
...
modifyProjectShare :: WizardRequestContextC s m => U.UUID -> ProjectShareChangeDTO -> m ProjectShareChangeDTO
...
modifyProjectSettings :: WizardRequestContextC s m => U.UUID -> ProjectSettingsChangeDTO -> m ProjectSettingsChangeDTO
...
deleteProject :: WizardRequestContextC s m => U.UUID -> Bool -> m ()
...
modifyContent :: WizardRequestContextC s m => U.UUID -> ProjectContentChangeDTO -> m ProjectContentChangeDTO
...
cleanProjects :: WizardRequestContextC s m => m ()
cleanProjects =
  runInTransaction $ do
...
cloneProjectEvents :: WizardRequestContextC s m => [ProjectEventList] -> m [ProjectEventList]
cloneProjectEvents oldEvents = do
  newEvents <- cloneProjectEventsWithOldEventUuid oldEvents
  return $ fmap snd newEvents

cloneProjectEventsWithOldEventUuid :: WizardRequestContextC s m => [ProjectEventList] -> m [(U.UUID, ProjectEventList)]
