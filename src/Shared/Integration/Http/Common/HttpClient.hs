module Shared.Integration.Http.Common.HttpClient (
  runRequest,
  runRequest',
  runRequestIO',
  runRequestIO'With,
...
runRequest :: RequestContextC s sc m => HttpRequest -> (Response BSL.ByteString -> Either AppError a) -> m a
...
runRequest' :: RequestContextC s sc m => HttpRequest -> (Response BSL.ByteString -> Either String a) -> m (Either String a)
runRequest' req responseMapper = do
  logRequestMultipart req
  eResponse <- runSimpleRequest req
  case eResponse of
...
runRequestIO' :: RequestContextC s sc m => HttpRequest -> (Response BSL.ByteString -> IO (Either String a)) -> m (Either String a)
...
runRequestIO'With :: RequestContextC s sc m => Manager -> HttpRequest -> (Response BSL.ByteString -> IO (Either String a)) -> m (Either String a)
...
runSimpleRequest :: RequestContextC s sc m => HttpRequest -> m (Either E.SomeException (Response BSL.ByteString))
runSimpleRequest req = do
  httpClientManager <- asks (.httpClientManager')
  runSimpleRequestWith httpClientManager req

runSimpleRequestWith :: RequestContextC s sc m => Manager -> HttpRequest -> m (Either E.SomeException (Response BSL.ByteString))
