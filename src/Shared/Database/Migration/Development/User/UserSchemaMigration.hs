module Shared.Database.Migration.Development.User.UserSchemaMigration where
...
createUserTable :: WizardRequestContextC s m => m Int64
createUserTable = do
...
createUserLocaleForeignKeyConstraint :: WizardRequestContextC s m => m Int64
...
createUserSubmissionPropsTable :: WizardRequestContextC s m => m Int64
...
createUserTokenTable :: WizardRequestContextC s m => m Int64
...
createUserGroupTable :: WizardRequestContextC s m => m Int64
...
createUserGroupMembershipTable :: WizardRequestContextC s m => m Int64
createUserGroupMembershipTable = do
  ...
  let sql =
        ...
        \); \
        \ \
        \CREATE INDEX user_group_membership_user_uuid_idx ON user_group_membership (user_uuid, tenant_uuid);"
  let action conn = execute_ conn sql
  runDB action

createUserTourTable :: WizardRequestContextC s m => m Int64
...
createUserPluginSettingsTable :: WizardRequestContextC s m => m Int64
