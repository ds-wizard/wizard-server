module Shared.Database.DAO.Project.ProjectEventDAO where
...
import Shared.Database.Mapping.Project.ProjectEvent ()
import Shared.Database.Mapping.Project.ProjectEventList ()
...
findProjectEventsPage :: WizardRequestContextC s m => U.UUID -> Pageable -> [Sort] -> m (Page ProjectEventList)
...
findProjectEventsByProjectUuid :: WizardRequestContextC s m => U.UUID -> m [ProjectEvent]
...
findProjectEventListsByProjectUuid :: WizardRequestContextC s m => U.UUID -> m [ProjectEventList]
...
findProjectEventByUuid :: WizardRequestContextC s m => U.UUID -> m ProjectEvent
...
findProjectEventByUuid' :: WizardRequestContextC s m => U.UUID -> m (Maybe ProjectEvent)
findProjectEventByUuid' uuid = do
  tenantUuid <- asks (.tenantUuid')
  createFindEntityByFn' entityName [tenantQueryUuid tenantUuid, ("uuid", U.toString uuid)]

insertProjectEvent :: WizardRequestContextC s m => ProjectEvent -> m Int64
insertProjectEvent = createInsertFn entityName

insertProjectEventWithTimestampUpdate :: WizardRequestContextC s m => U.UUID -> ProjectEvent -> m ()
...
insertProjectEvents :: WizardRequestContextC s m => [ProjectEvent] -> m Int64
...
updateProjectEventByUuid :: WizardRequestContextC s m => ProjectEvent -> m Int64
...
syncProjectEventsWithDb :: WizardRequestContextC s m => [ProjectEvent] -> [ProjectEvent] -> m ()
...
deleteProjectEvents :: WizardRequestContextC s m => m Int64
deleteProjectEvents = createDeleteEntitiesFn entityName

deleteProjectEventsByUuids :: WizardRequestContextC s m => [U.UUID] -> m ()
deleteProjectEventsByUuids eventUuids =
  unless
    (null eventUuids)
    (void $ createDeleteEntityWhereInFn entityName "uuid" (fmap U.toString eventUuids))

deleteProjectEventsByProjectUuid :: WizardRequestContextC s m => U.UUID -> m Int64
