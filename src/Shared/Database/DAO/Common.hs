...
import Shared.Util.String (replace, toSnake, trim)

runDB :: RequestContextC s sc m => (Connection -> IO b) -> m b
runDB action = do
  context <- ask
...
runDBImmediately :: RequestContextC s sc m => (Connection -> IO b) -> m b
runDBImmediately action = do
  context <- ask
  handleSqlErrorIfPresent $ withResource context.dbPool' action

runRawDB :: RequestContextC s sc m => (LibPQ.Connection -> IO b) -> m b
runRawDB action = do
  context <- ask
  case context.dbConnection' of
...
handleSqlErrorIfPresent :: RequestContextC s sc m => IO b -> m b
...
runOneEntityDB :: RequestContextC s sc m => String -> (Connection -> IO [b]) -> [(String, String)] -> m b
...
runOneEntityDB' :: RequestContextC s sc m => String -> (Connection -> IO [b]) -> [(String, String)] -> m (Maybe b)
runOneEntityDB' entityName action queryParams = do
  entities <- runDB action
  case entities of
    [] -> return Nothing
...
logQuery :: (RequestContextC s sc m, ToRow q) => Query -> q -> m ()
...
logInsertAndUpdate :: (RequestContextC s sc m, ToRow q) => Query -> q -> m ()
...
runInTransaction :: RequestContextC s sc m => (String -> String -> m ()) -> (String -> String -> m ()) -> m a -> m a
...
createFindEntitiesFn :: (RequestContextC s sc m, FromRow entity) => String -> m [entity]
createFindEntitiesFn = createFindEntitiesWithFieldsFn "*"

createFindEntitiesWithFieldsFn :: (RequestContextC s sc m, FromRow entity) => String -> String -> m [entity]
...
createFindEntitiesSortedFn :: (RequestContextC s sc m, FromRow entity) => String -> [Sort] -> m [entity]
createFindEntitiesSortedFn = createFindEntitiesWithFieldsSortedFn "*"

createFindEntitiesWithFieldsSortedFn :: (RequestContextC s sc m, FromRow entity) => String -> String -> [Sort] -> m [entity]
...
createFindEntitiesByFn :: (RequestContextC s sc m, FromRow entity) => String -> [(String, String)] -> m [entity]
...
createFindEntitiesWithFieldsByFn :: (RequestContextC s sc m, FromRow entity) => String -> String -> [(String, String)] -> m [entity]
...
createFindEntitiesBySortedFn :: (RequestContextC s sc m, FromRow entity) => String -> [(String, String)] -> [Sort] -> m [entity]
createFindEntitiesBySortedFn = createFindEntitiesWithFieldsBySortedFn "*"

createFindEntitiesWithFieldsBySortedFn :: (RequestContextC s sc m, FromRow entity) => String -> String -> [(String, String)] -> [Sort] -> m [entity]
...
createFindEntityByFn :: (RequestContextC s sc m, FromRow entity) => String -> [(String, String)] -> m entity
createFindEntityByFn = createFindEntityWithFieldsByFn "*" False

createFindEntityWithFieldsByFn :: (RequestContextC s sc m, FromRow entity) => String -> Bool -> String -> [(String, String)] -> m entity
...
createFindEntityByFn' :: (RequestContextC s sc m, FromRow entity) => String -> [(String, String)] -> m (Maybe entity)
createFindEntityByFn' = createFindEntityWithFieldsByFn' "*"

createFindEntityWithFieldsByFn' :: (RequestContextC s sc m, FromRow entity) => String -> String -> [(String, String)] -> m (Maybe entity)
...
createFindEntitiesPageableQuerySortFn
  :: (RequestContextC s sc m, ToRow q, FromRow entity)
  => String
  -> String
  -> Pageable
  -> [Sort]
...
createFindColumnBySqlPageFn
  :: (RequestContextC s sc m, FromField entity)
  => String
  -> Pageable
  -> Query
  -> [String]
...
createFindEntitiesInFn :: (RequestContextC s sc m, FromRow entity) => String -> U.UUID -> String -> [String] -> m [entity]
...
createInsertFn :: (RequestContextC s sc m, ToRow entity) => String -> entity -> m Int64
...
createInsertWithoutTransactionFn :: (RequestContextC s sc m, ToRow entity) => String -> entity -> m Int64
...
createDeleteEntitiesFn :: RequestContextC s sc m => String -> m Int64
...
createDeleteEntitiesByFn :: RequestContextC s sc m => String -> [(String, String)] -> m Int64
...
createDeleteEntityWhereInFn :: RequestContextC s sc m => String -> String -> [String] -> m Int64
...
createDeleteEntityByFn :: RequestContextC s sc m => String -> [(String, String)] -> m Int64
...
createCountFn :: RequestContextC s sc m => String -> m Int64
...
createCountByFn :: (RequestContextC s sc m, ToRow q) => String -> String -> q -> m Int
...
createCountWithSqlFn :: RequestContextC s sc m => Query -> [String] -> m Int
...
createCountByWithComputedEntityFn :: (RequestContextC s sc m, ToRow q) => String -> String -> q -> m Int
...
createSumByFn :: (RequestContextC s sc m, ToRow q) => String -> String -> String -> q -> m Int64
