module Shared.Util.Sentry where

import Control.Concurrent (forkIO)
import Control.Exception (Exception, SomeException (..), fromException, someExceptionContext, throwIO)
...
isIgnoredException wrapped =
  let exception = unwrapException wrapped
...
data RequestContextException = RequestContextException
  { appException :: SomeException
  , appBreadcrumbs :: [SentryBreadcrumb]
  , appIdentity :: Maybe String
  , appTraceUuid :: String
  , appTenantUuid :: String
  }

instance Show RequestContextException where
  show carried = show carried.appException

instance Exception RequestContextException

throwWithRequestContext :: RequestContextException -> IO a
throwWithRequestContext = throwIO

unwrapException :: SomeException -> SomeException
unwrapException exception =
  case toRequestContextException exception of
    Just carried -> carried.appException
    Nothing -> exception

toRequestContextException :: SomeException -> Maybe RequestContextException
toRequestContextException = fromException

toGeneralServerErrorEvent :: String -> SentryEvent
toGeneralServerErrorEvent message =
...
sentryOnException target getSentryIdentity mRequest exception
  ...
  | otherwise = do
      ...
      defaultOnException mRequest (unwrapException exception)

toWebServerEvent :: (Maybe String -> [(String, Value)]) -> Maybe WAI.Request -> SomeException -> SentryEvent
toWebServerEvent getSentryIdentity mRequest wrapped =
  let event = toRequestEvent getSentryIdentity mRequest (unwrapException wrapped)
   in case toRequestContextException wrapped of
        Just carried -> enrichWithRequestContext event carried
        Nothing -> event

enrichWithRequestContext :: SentryEvent -> RequestContextException -> SentryEvent
enrichWithRequestContext event carried =
  event
    { breadcrumbs = carried.appBreadcrumbs
    , tags = event.tags ++ [("traceUuid", carried.appTraceUuid), ("tenantUuid", carried.appTenantUuid)]
    , interfaces = event.interfaces ++ toUserInterface carried.appIdentity Nothing ++ toTraceInterface carried.appTraceUuid
    }

toRequestEvent :: (Maybe String -> [(String, Value)]) -> Maybe WAI.Request -> SomeException -> SentryEvent
toRequestEvent _ Nothing exception =
...
toRequestEvent getSentryIdentity (Just request) exception =
