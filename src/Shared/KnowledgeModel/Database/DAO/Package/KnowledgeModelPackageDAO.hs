...
findPackagesByOrganizationIdAndKmId organizationId kmId = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesByFn table [tenantQueryUuid tenantUuid, ("organization_id", organizationId), ("km_id", kmId)]
...
findPackagesByPreviousPackageUuid previousPackageUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesByFn table [tenantQueryUuid tenantUuid, ("previous_package_uuid", U.toString previousPackageUuid)]
...
findPackagesByForkOfPackageId forkOfPackageId = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntitiesByFn table [tenantQueryUuid tenantUuid, ("fork_of_package_id", forkOfPackageId)]
...
findPackagesByUnsupportedMetamodelVersion metamodelVersion = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  let sql = fromString $ f' "SELECT * FROM %s WHERE metamodel_version != ? AND tenant_uuid = ?" [table]
...
findSeriesOfPackagesRecursiveByUuid pkgUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  eventTable <- tableName "knowledge_model_package_event"
  let sql =
        fromString $
          f''
            ...
            \FROM recursive \
            \ORDER BY level DESC;"
            [("package", table), ("packageEvent", eventTable)]
...
findUsablePackagesForDocumentTemplate dtUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  documentTemplateTable <- tableName "document_template"
  let sql =
        fromString $
          f''
            ...
            [("package", table), ("documentTemplate", documentTemplateTable)]
...
findVersionsForPackage orgId kmId = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  let sql = fromString $ f' "SELECT version FROM %s WHERE tenant_uuid = ? and organization_id = ? and km_id = ?" [table]
...
findPackageByCoordinate Coordinate {..} = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createFindEntityByFn table [tenantQueryUuid tenantUuid, ("organization_id", organizationId), ("km_id", entityId), ("version", version)]
...
findLatestPackageByOrganizationIdAndKmId orgId kmId mPhase = do
  ...
  runOneEntityDB table action ([("organization_id", orgId), ("km_id", kmId)] ++ phaseParams)
  ...
  let sql =
        fromString $
          f''
            ...
            \LIMIT 1"
            [("package", table), ("phaseCondition", phaseCondition)]
  let params = [U.toString tenantUuid, orgId, kmId]
  logQuery sql params
...
countPackages = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createCountByFn table tenantCondition [tenantUuid]
...
deletePackagesFiltered queryParams = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createDeleteEntitiesByFn table (tenantQueryUuid tenantUuid : queryParams)
...
deletePackageByCoordinate Coordinate {..} = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createDeleteEntityByFn table [tenantQueryUuid tenantUuid, ("organization_id", organizationId), ("km_id", entityId), ("version", version)]
