...
findPackageEvents pkgUuid = do
  ...
  createFindEntitiesWithFieldsBySortedFn "*" table [tenantQueryUuid tenantUuid, ("package_uuid", U.toString pkgUuid)] [Sort "created_at" Ascending]
...
deletePackageEventsByPackageUuid pkgUuid = do
  tenantUuid <- asks (.tenantUuid')
  table <- tableName entityName
  createDeleteEntityByFn table [tenantQueryUuid tenantUuid, ("package_uuid", U.toString pkgUuid)]
