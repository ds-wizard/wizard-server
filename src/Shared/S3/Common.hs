...
createGetObjectFn :: RequestContextC s sc m => String -> m BS.ByteString
...
createGetObjectConduitActionFn :: RequestContextC s sc m => String -> m (Minio (C.ConduitM () BS.ByteString Minio ()))
...
createGetObjectWithTenantFn :: RequestContextC s sc m => U.UUID -> String -> m BS.ByteString
...
createGetObjectFn' :: RequestContextC s sc m => String -> m (Either MinioErr BS.ByteString)
createGetObjectFn' object = do
  bucketName <- getBucketName
  sanitizedObject <- sanitizeObject object
  logInfoI _CMP_S3 (f' "Get object: '%s'" [sanitizedObject])
...
createPutObjectFn :: RequestContextC s sc m => String -> Maybe String -> Maybe String -> BS.ByteString -> m String
...
createPutObjectWithTenantFn :: RequestContextC s sc m => U.UUID -> String -> Maybe String -> Maybe String -> BS.ByteString -> m String
...
createPutObjectConduitFn :: RequestContextC s sc m => String -> Maybe String -> Maybe String -> Minio (C.ConduitM () BS.ByteString Minio ()) -> m String
...
createRemoveObjectFn :: RequestContextC s sc m => String -> m ()
...
createRemoveObjectWithTenantFn :: RequestContextC s sc m => U.UUID -> String -> m ()
...
createListObjectsFn :: RequestContextC s sc m => m [String]
...
createBucketExistsFn :: RequestContextC s sc m => m Bool
...
createMakeBucketFn :: RequestContextC s sc m => m ()
...
createPurgeBucketFn :: RequestContextC s sc m => m ()
...
createRemoveBucketFn :: RequestContextC s sc m => m ()
...
createSetBucketPolicyFn :: RequestContextC s sc m => String -> String -> m ()
...
makeBucketPublicReadOnly :: RequestContextC s sc m => m ()
...
createPresignedGetObjectUrl :: RequestContextC s sc m => String -> Int -> m String
...
createMakePublicLink :: RequestContextC s sc m => String -> m String
...
createMakePublicLinkWithTenantFn :: RequestContextC s sc m => U.UUID -> String -> m String
...
runMinioClient :: RequestContextC s sc m => Minio a -> m a
...
runMinioClient' :: RequestContextC s sc m => Minio a -> m (Either MinioErr a)
runMinioClient' action = do
  context <- ask
  let s3Client = context.s3Client'
  liftIO $ runMinioWith s3Client action

getS3Url :: RequestContextC s sc m => m String
getS3Url = do
  context <- ask
  return context.serverConfig'.s3'.url

getBucketName :: RequestContextC s sc m => m String
getBucketName = do
  context <- ask
  return context.serverConfig'.s3'.bucket

sanitizeObject :: RequestContextC s sc m => String -> m String
...
sanitizeObjectWithTenant :: RequestContextC s sc m => U.UUID -> String -> m String
...
buildObjectUrl :: RequestContextC s sc m => String -> m String
