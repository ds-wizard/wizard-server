module Specs.Api.Handler.Project.Detail_Settings_GET (
  detail_settings_GET,
) where
...
import Shared.Model.KnowledgeModel.KnowledgeModel hiding (request)
...
test_200 requestContext = do
  ...
  create_test_200
    ...
    []
  create_test_200'
    "HTTP 200 OK (Document Template without Locales)"
    requestContext
    project1
    [reqAuthHeader]
    [project1AlbertEditProjectPermDto]
    (runInContextIO deleteDocumentTemplateLocales requestContext)
    []

create_test_200 title requestContext project authHeader permissions =
  create_test_200' title requestContext project authHeader permissions (return ()) [czechWizardDocumentTemplateLocaleList]

create_test_200' title requestContext project authHeader permissions extraSetup locales =
  it title $
    ...
    do
      ...
      let expDto =
            ProjectDetailSettings
              ...
              , availableLocales = []
              , documentTemplate = Just $ toSuggestionDTO' wizardDocumentTemplate wizardDocumentTemplateFormats locales
