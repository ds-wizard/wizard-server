module Specs.Service.KnowledgeModel.Migration.Migrator.SanitizerSpec where
...
import Specs.Common
import Specs.Service.KnowledgeModel.Migration.Migrator.Common

sanitizerSpec requestContext =
  describe "Sanitizer" $
    -- -------------------------------------------------------------
    -- -------------------------------------------------------------
    do
      describe "Sanitize EditKnowledgeEvent" $ do
        it "Event - all KM uuids exists, no new added in event" $
          ...
          do
            ...
            -- When:
            (Right resState) <- runInContext (migrate reqState) requestContext
            -- Then:
