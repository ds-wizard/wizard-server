module Specs.Service.Project.ProjectAclSpec where
...
projectAclSpec requestContext =
  describe "Project ACL" $ do
    ...
    let makeContext user = requestContext {currentUser = Just . U_Mapper.toDTO $ user}
    ...
    let anonymousCtx = requestContext {currentUser = Nothing}
    it "checkViewPermissionToProject" $ do
      runLocalTestMigration requestContext
...
runLocalTestMigration requestContext = do
  runInContext deleteProjectPerms requestContext
  runInContext deleteProjects requestContext
  runInContext (insertPackage germanyKmPackage) requestContext
