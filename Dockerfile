...
RUN apt-get update \
 && apt-get -qq -y install libmemcached-dev ca-certificates netbase curl libpq5 libpq-dev python3-dev libxml2 \
...
# Add dist folder with the binary, config and native libraries - one dist per
# target architecture, each built natively (TARGETARCH is amd64 or arm64)
ARG TARGETARCH
ADD dist-$TARGETARCH /app/
